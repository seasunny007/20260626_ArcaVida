...
| **核心能力** | 多源数据导入/抓取 -> 地址转坐标 -> 地图可视化 -> 置信度标注 -> 简报生成；已落地本地物资发放追踪 |
...
| 物资送达和缺口信息需要可视化追踪                         | `core/material_manager.py`、Web 物资/地图/导出/审计入口 | P2 已落地 |
...
实际推进节奏以安全、隐私、测试、部署条件和人工复核流程成熟度为准；本文只定义开发顺序，不承诺固定天数。
