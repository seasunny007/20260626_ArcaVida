...
- 提供本地物资管理、地图看板、脱敏导出和审计日志，覆盖安置点/发放点的物资需求、库存、送达记录和缺货指数。
- Ofrece gestion local de materiales, mapa, exportacion anonimizada y auditoria, cubriendo necesidades, inventario, entregas e indice de escasez para puntos de alojamiento o distribucion.
- Provides local material management, map board, sanitized export, and audit logs, covering needs, inventory, delivery records, and shortage indexing for shelter or distribution points.
...
- 物资管理模块只提供信息整理、缺口排序、地图提示和脱敏导出，不提供自动派送、路线选择或自动分配指令。
- El modulo de gestion de materiales solo ofrece organizacion de informacion, ranking de faltantes, mapa y exportacion anonimizada; no proporciona envio automatico, seleccion de rutas ni instrucciones automaticas de asignacion.
- The material management module provides information organization, shortage ranking, map hints, and sanitized export only; it does not provide automatic delivery, route selection, or automatic allocation instructions.
...
- 物资管理：参考 `docs/material-distribution-tracking.md`，通过本地 Web 工作台使用点位、需求、送达、地图、导出和审计功能。
- Gestion de materiales: consulta `docs/material-distribution-tracking.md` y usa la estacion web local para puntos, necesidades, entregas, mapa, exportacion y auditoria.
- Material management: see `docs/material-distribution-tracking.md` and use the local web workstation for points, needs, deliveries, map, export, and audit features.
